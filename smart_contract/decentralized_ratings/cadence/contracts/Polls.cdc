pub struct Polls {
    pub struct Poll {
        ...
        init(poll_id: str, voting_deadline: UInt64, max_options: UInt8, public_key: String) {
        ...
        pub fun reveal_poll(result: String, secret_key: String) {
    ...
    pub struct Content {
        ...
        pub fun add_vote(rating: UInt8) {
            self.total_revealed_votes = self.total_revealed_votes + 1
            self.total_revealed_rating = self.total_revealed_rating + vote.decrypted_vote!
        }
    ...
    pub fun addVote(poll_id: String, encrypted_vote: String) {
        self.votes[getSender()] = Vote(poll_id: poll_id, encrypted_vote: encrypted_vote)
        self.polls[poll_id].addVote()
    }

    pub fun addStake(poll_id: String, staked_amount: UInt64, range_begin: UInt8, range_end: UInt8) {
        self.stakes[getSender()] = Stake(poll_id: poll_id, staked_amount: staked_amount, range_begin: range_begin, range_end: range_end)
    }
    ...
    pub fun revealPoll(poll_id: String, result: String, secret_key: String) {
    ...
    pub fun revealVote(decrypted_vote: UInt8) {
        self.votes[getSender()].reveal_vote(decrypted_vote: decrypted_vote)
    }
}